...
public class WrongedRenderer extends GeoEntityRenderer<WrongedEntity> {
            ...
            for (int i = 1; i < 4; i++) {
                if (randomNumber == i) {
                    if (isNighttime || entity.level().isNight() || entity.level().getDayTime() > 13000)
                        return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged_fedora_night.png");
                    else return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged_fedora.png");
                } else if (i == 3) {
                    if (randomNumber == 4 || randomNumber == 5)
                        if (isNighttime || entity.level().isNight() || entity.level().getDayTime() > 13000)
                            return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged_emo_night.png");
                        else return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged_emo.png");
                }
            }
        }
    ...
    public ResourceLocation pickDefaultTexture (WrongedEntity entity) {
        if (isNighttime || entity.level().isNight() || entity.level().getDayTime() > 13000) {
            return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged_night.png");
        }
        else
            return ResourceLocation.fromNamespaceAndPath(Starless.MODID, "textures/entity/wronged.png");
    }
